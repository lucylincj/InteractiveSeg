...
#define atEdgeMap(x,y) edgeMap[(x)+(y)*width]
  ...
  maxlab = vl_malloc(sizeof(float) * numRegions ) ;
  distlab = vl_malloc(sizeof(float) * width * height * numChannels);
  ...
  for (iter = 0 ; iter < maxNumIterations ; ++iter) {
    ...
    float factor = 1 / (regionSize * regionSize) ;
    float energy = 0 ;

    //-----------------------------------------------------------------
    // Assign the max color distance for a cluster 2014.04.16 jttte
    //-----------------------------------------------------------------

    //erase max distance in previous calculation
    

    if(iter == 0)
      memset(maxlab, sqrt(regularization), sizeof(float) * numRegions) ;
    else {
      memset(maxlab, 0, sizeof(float) * numRegions) ;
      int index;
      for (y = 0 ; y < (signed)height ; ++y) {
        for (x = 0 ; x < (signed)width ; ++x) {
          index = x + y * width;
          int cluster = segmentation[index];
          if(maxlab[cluster] < distlab[index]) maxlab[cluster] = distlab[index];
        ...
        for (vp = VL_MAX(0, v) ; vp <= VL_MIN((signed)numRegionsY-1, v+1) ; ++vp) {
          for (up = VL_MAX(0, u) ; up <= VL_MIN((signed)numRegionsX-1, u+1) ; ++up) {
            ...
            for (k = 0 ; k < (signed)numChannels ; ++k) {
              ...
              float d = (z - centerz) * (z - centerz) ;

              //float appearance += (z - centerz) * (z - centerz) ;
              appearance += d ;
            }
            // record the distance, so we can update maxlab later
            distlab[ x + y * width ] = appearance;
            distance = appearance + factor * spatial * maxlab[x + y * width] * maxlab[x + y * width];
